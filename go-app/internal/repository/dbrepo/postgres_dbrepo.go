...
func (m *PostgresDBRepo) AllMovies(genre ...int) ([]*models.Movie, error) {
	...
	query := fmt.Sprintf(`
		select
			id, title, release_date, runtime, description, coalesce(image, ''),
...
func (m *PostgresDBRepo) InsertMovie(movie models.Movie) (int, error) {
	...
	stmt := `insert into movies (title, description, release_date, runtime, created_at, updated_at, image)
...
func (m *PostgresDBRepo) UpdateMovie(movie models.Movie) error {
	...
	stmt := `update movies set title = $1, description = $2, release_date = $3, runtime = $4, updated_at = $5, image = $6 
			where id = $7`
